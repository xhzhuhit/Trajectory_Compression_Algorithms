...
ErrorAndRate::ErrorAndRate(pointSeq pS,double compression_rate,int compression_time,double priority_limit)
{
	...
	for(int p = 0;p < pS.store.size();p++)
	{
		...
		if(semi_buffer_size == int(compression_time * 60 / rate) + 2 || p == pS.store.size()-1)//+2很重要！！
		{
			...
			//寻找最小优先级
			int weizhi = 1;
			double youxianji = semi_buffer[1].priority;//卧槽，位置设置有问题！！
			for(int j = 1;j < semi_buffer.size()-1;j++)
			{
...
ErrorAndRate::~ErrorAndRate(void)
{
}

pair<double,double> ErrorAndRate::getLocationL(double l)
{
	int i;
...
pair<double,double> ErrorAndRate::getLocationT(double t)
{
	int i;
	for(i = 0; i < number - 1; i ++)
	{
...
void ErrorAndRate::calcDistance()
{
	double a = 0;
	int start = 0;
	int c =0;
